...
<div class="modal-body flex justify-center items-center h-screen">
  <form class="w-full max-w-md" action="/update_book/{{$books['id']}}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-4">
      <input type="text" placeholder="Enter book category" class="px-4 py-2 rounded-lg border-gray-300 focus:border-blue-500 bg-white w-full" value="{{$books['id_category']}}" name="id_category">
      @error('id_category')
          <p class="text-red-500 text-xs mt-1">{{$message}}</p>
      @enderror
    </div>
